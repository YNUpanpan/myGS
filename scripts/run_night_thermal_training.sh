...
DEFAULT_SCENE="night_thermal"
TRAINING_SCENE="${MYGS_NIGHT_THERMAL_SCENE:-${DEFAULT_SCENE}}"
...
ensure_no_active_night_thermal_run() {
  local pid_file pid
  ...
  while IFS= read -r pid_file; do
    ...
    if [[ "${pid}" =~ ^[0-9]+$ ]] && kill -0 "${pid}" 2>/dev/null; then
      echo "Night thermal training process is already alive: pid=${pid} file=${pid_file}" >&2
      exit 61
    fi
