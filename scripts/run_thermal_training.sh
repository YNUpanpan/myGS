...
SOURCE_DIR="$(scene_processed_dir thermal)"
...
ensure_no_active_thermal_run() {
  local pid_file pid
  if [[ ! -d "$(scene_output_root thermal)" ]]; then
    return
  fi
  while IFS= read -r pid_file; do
    ...
    if [[ "${pid}" =~ ^[0-9]+$ ]] && kill -0 "${pid}" 2>/dev/null; then
      echo "Thermal training process is already alive: pid=${pid} file=${pid_file}" >&2
      exit 61
    fi
  done < <(find "$(scene_output_root thermal)" -mindepth 2 -maxdepth 2 -type f -name train.pid -print)
}
...
preflight() {
  ...
  prepared="$(find "${SOURCE_DIR}/images" -maxdepth 1 \( -type f -o -type l \) -name "$(scene_pattern thermal)" | wc -l)"
  test "${prepared}" = "339"
...
new_run() {
  ...
  output_dir="$(scene_output_root thermal)/${run_id}"
  log_file="$(log_path train thermal)"
...
resume_run() {
  ...
  case "${run_dir}" in
    ...
    *) echo "Resume directory is outside thermal output root: ${run_dir}" >&2; exit 63 ;;
  esac
